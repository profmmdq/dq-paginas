(function () {
  ...
  /* ─── Campos congelados (pagina quiz/mentoria — ordem 1..18) ─── */
  var FIELDS = [
  ...
  function doSubmit() {
    ...
    fetch(SB + "/functions/v1/api-formulario", {
      ...
      try {
        ...
        window.dataLayer.push({ event: "DiagnosticCompleted", content_name: "quiz/mentoria", form_slug: "conselho" });
      } catch (x) {}
      showSuccess();
    }).catch(function (e) {
      clearTimeout(to);
  ...
  function boot() {
    ...
    try {
      fetch(SB + "/rest/v1/rpc/fn_registrar_visita", {
        ...
        body: JSON.stringify({ p_pagina: "quiz/mentoria" })
      }).catch(function () {});
    } catch (x) {}
  }
